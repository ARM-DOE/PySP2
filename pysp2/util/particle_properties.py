...
def calc_diams_masses(input_ds, debug=True):
    ...
    rejectFtPosTotal = 0
    Globals = DMTGlobals()
    PkHt_ch0 = np.max(np.stack([input_ds['PkHt_ch0'].values, input_ds['FtAmp_ch0'].values]), axis=0)
    PkHt_ch4 = np.max(np.stack([input_ds['PkHt_ch4'].values, input_ds['FtAmp_ch4'].values]), axis=0)
    accepted = np.logical_and.reduce((PkHt_ch0 > Globals.ScatMinPeakHt,
    ...
    rejectMinScatTotal += np.sum(PkHt_ch0 < Globals.ScatMinPeakHt)
    ...
    accepted_incand = np.logical_and(input_ds['PkHt_ch1'].values >= Globals.IncanMinPeakHt,
                                     input_ds['PkHt_ch2'].values >= Globals.IncanMinPeakHt)
    ...
    accepted_incand = np.logical_and.reduce(
        (accepted_incand, ~np.logical_and(input_ds['PkHt_ch1'].values > Globals.IncanMaxPeakHt,
    ...
    accepted_incand = np.logical_and.reduce(
        (accepted_incand, ~np.logical_and.reduce((
            input_ds['PkHt_ch1'].values > Globals.IncanMaxPeakHt, input_ds['PkHt_ch5'].values < Globals.IncanMaxPeakHt,
    ...
    peak_loc1_reject = np.logical_and.reduce((accepted_incand, input_ds['PkHt_ch1'].values < Globals.IncanMaxPeakHt,
    ...
    peak_loc5_reject = np.logical_and.reduce(
        (accepted_incand, input_ds['PkHt_ch1'].values > Globals.IncanMaxPeakHt,
         input_ds['PkHt_ch5'].values < Globals.IncanMaxPeakHt,
    ...
    Scatter = np.where(PkHt_ch0 < Globals.ScatMaxPeakHt, Scat_not_sat, Scat_sat)
    Scatter[~accepted] = np.nan
    ...
    output_ds['sootMass'] = (('event_index'),
                             np.where(input_ds['PkHt_ch1'] > Globals.IncanMaxPeakHt, sootMass_sat, sootMass_not_sat))
    output_ds['sootDiam'] = (('event_index'),
                             np.where(input_ds['PkHt_ch1'] > Globals.IncanMaxPeakHt, sootDiam_sat, sootDiam_not_sat))
