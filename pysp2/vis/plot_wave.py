...
def plot_wave(ds, record_no, chn, plot_fit=True, append_to_display=False,init_kwargs=None, **kwargs):
    """
...
def plot_waves(ds,record_no,plot_fit=True):
    ...
    legends=[['SCHG ch0','ch0_fit','SCLG ch4','ch4_fit'],['BBHG ch1','NBHG ch2','BBLG ch5','NBLG ch6'],['ch3 HG','CH7 LG']]
    ...
    for i,panel in enumerate(panel_number):
        if i!=0:
            display=plot_wave(ds,record_no,chns[i],plot_fit=plot_fit,
                                    ...
                                    subplot_index=(panel,))
    titles=['Scattering chanels','Incandesence channels','Split detector channels']
    xlabels=['','','Bin #']
    ylabels=['Data_ch0, Data_ch4','Data_ch1, Data_ch2\nData_ch5, Data_ch6','Data_ch3, Data_ch7']
